...
let URL = "https://teachablemachine.withgoogle.com/models/w-vCEgD_B/";
...
function checkPose(prediction, video) {
   ...
   if (prob > 0.9 && !explosionActive) {
        const poseState = poseStates[`pose${poseNumber}`];
        
        switch(poseNumber) {
            case '1':
                if (time >= 4.5 && time <= 5.5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >= 9 && time <= 10 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
                if (time >= 13.5 && time <= 14.5 && !poseState.firstWindowTriggered){
                    triggerExplosion(poseState);
                }
                break;
            case '4':
                if (time >= 20 && time <= 21.9 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 24 && time <= 25 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/w-vCEgD_B/");
    }
}
